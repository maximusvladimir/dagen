...
public class FreedomCastle extends JFrame
{
    ...
    BufferedImage terrain, treeBuffer, nickiMinaj, treeShadow, map, all;
    ...
    public void init()
    {
        ...
        all = new BufferedImage(800, 600, BufferedImage.TYPE_INT_RGB);
        for (int v = 0; v < (int)(Math.random() * 7) + 2; v++)
        {
            ...
            river.generate(MapType.Hills, (int)(Math.random() * 255), 1.0);
            rivers.add(river);
        }
        ...
        for (int xtt = 0; xtt < 800; xtt += 2)
        {
            for (int ytt = 600; ytt > 0; ytt -= 2)
            {
                ...
                else
                {
                    ...
                    gt.setColor(c);
                    grassRand[x][y] = new Color(rtc, gtc, btc);
